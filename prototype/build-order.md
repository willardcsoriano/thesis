...
- **Delivers:**
  ...
  - Bounded multi-step loop (D21): after each executed command, its result (stdout/stderr/exit code) is fed back to the model, which proposes the next command toward the same task or signals completion; every step is independently classified and gated, no batch approval; a hard step cap prevents runaway iteration — **not yet built**
...
- **Done when:** `go run ./cmd/synapse` prints a proposed command for all sample tasks (✅ demonstrated); `synapse "<task>"` proposes a command, classifies it, auto-runs reversible commands and shows their output, and blocks irreversible commands on confirmation before running them (✅ demonstrated live, all three paths); a downed server prints an actionable error instead of a stack trace (✅, via `client.Ping`); a genuinely multi-step task (requiring more than one command) completes correctly through the bounded loop, with every step individually gated and a step-limit case reported honestly rather than silently truncated (not yet demonstrated).
- **Status:** Single-command propose/classify/execute complete and validated live 2026-07-12 (see history below). Bounded multi-step loop (D21) is the actual next work.
...
- **Risk not yet retired (D21):** Whether the model can reliably signal loop completion (vs. looping until the hard cap) and whether feeding execution errors back actually produces better follow-up commands rather than just repeating the same mistake differently worded.
