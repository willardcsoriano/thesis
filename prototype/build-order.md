...
- **Delivers:**
  ...
  - Bounded multi-step loop (D21): after each executed command, its result (stdout/stderr/exit code) is fed back to the model, which proposes the next command toward the same task or signals completion; every step is independently classified and gated, no batch approval; a hard step cap prevents runaway iteration — **done, validated live**
