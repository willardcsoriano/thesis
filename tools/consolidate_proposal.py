#!/usr/bin/env python3
"""Consolidate Chapters 1, 2, and 3 into a Mapua University writing-guidelines compliant monograph."""
import os
import re
import subprocess
...
OUT_PATH = SUBMISSIONS / "consolidated" / "SynapseOS_Proposal_Chapters_1_to_3.html"
PDF_PATH = SUBMISSIONS / "consolidated" / "SynapseOS_Proposal_Chapters_1_to_3.pdf"
...
def extract_appendices(html_str):
    ...
    the consolidated References section instead of before it. Chapter 3's own
    References section immediately precedes Appendices, and strip_references_section()
    only removes up to the *next* h2 tag -- if Appendices weren't extracted first,
    its opening <div> would be stripped along with References, leaving an orphaned
    closing </div> that corrupts the surrounding DOM structure.
    """
...
def strip_references_section(html_str):
    ...
    N's own References list, including its closing tag. Each ref-entry inside is
    itself a <div>, so the match must consume exactly one <div class="ref-entry">
    per iteration rather than doing a naive non-greedy match to the first </div>
    (which would stop at the first reference entry and orphan the outer closing tag).
    """
    pattern = re.compile(
...
def compile_pdf():
    ...
    subprocess.run(
        [str(ROOT / "tools" / "venv" / "bin" / "python"), str(ROOT / "tools" / "export_pdf.py"), str(OUT_PATH), str(PDF_PATH)],
        env=env,
        check=True
    )
...
def detect_page_numbers():
    ...
    # Search for headings across the whole document; exact-line matching (for the
    # keys in EXACT_LINE_KEYS) avoids false positives from the TOC/LOT/LOF entries
    # and from in-body prose that happens to mention the same words.
    targets = {
...
def main():
  ...
  th {
    ...
    # Insert CSS rules for page layout
...
"""
    ...
    # Approval / Acknowledgement / Abstract front matter, per the writing-guidelines
    # Section F sample TOC order: Title, Approval, Acknowledgement, TOC, LOT, LOF, Abstract.
    approval_page = """
...
"""
    ...
    def make_doc_content(pages_dict):
        # Format Master TOC and Lists with exact page numbers
...
<div class="toc-block">
  ...
  <p class="toc-entry level-2"><a href="#sec-notes">Notes</a><span class="toc-page">{p_ch1 + 4}</span></p>
...
<head>
<meta charset="UTF-8">
<title>SynapseOS: Research Proposal (Chapters 1–3)</title>
{style}
<style>
  .chapter-break {{
    page-break-before: always;
...
"""
    ...
    compile_pdf()
    print("Pass 2 complete. Master proposal PDF successfully compiled.")
