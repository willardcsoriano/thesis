...
| Go / Ollama / bubbletea / lipgloss APIs | `prototype/` | These libraries change API surface fast, and the Charm v2 line already broke several signatures this project now depends on (`Model.View()` returns `tea.View` not `string`; `tea.KeyPressMsg` not `tea.KeyMsg`; lipgloss's renderer/color-profile API removed entirely) | Verify current API shape via Context7 or official docs before writing against a part of the surface not already used here — and prefer the compiler and a real `go get` over documentation, which has repeatedly lagged reality for these packages |
| Charm TUI module paths (`charm.land/{bubbletea,lipgloss,bubbles}/v2`) | `prototype/go.mod`, `internal/tui` | The v2 line moved off `github.com/charmbracelet/…` entirely — `go get github.com/charmbracelet/bubbletea/v2` fails outright, and published docs (including Context7) still show the old URLs. A path that moved once can move again | Trust `go get` and the module proxy over any documentation URL; if a Charm import stops resolving, check the module's own `go.mod` for its declared path before assuming the package is gone. Also pins Go ≥ 1.25 |
| Ollama's native tool-calling (`tool_calls`) never populates for `qwen2.5-coder` | `internal/typedops`, `internal/ollama.Chat` | Empirically confirmed only for this specific model family/version (F4, 0/20 across the full test matrix) — a future Ollama release or a different model tag could change this | Recheck before ever wiring `internal/typedops` into `runLoop`'s default path, and on any Ollama version upgrade — don't assume the freeform-JSON fallback is still necessary without re-testing `tool_calls` first |
