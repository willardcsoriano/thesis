...
- `[x]` **CLI mode** — one-shot invocation (`synapse "<task>"` → proposed command → classify → auto-run or confirm → execute → exit), no persistent session; formalized as a permanent shipped mode rather than TUI scaffolding (D19). **Done and validated live 2026-07-12** — all three stages (propose, classify, execute) demonstrated against Ollama with real auto-run, block-and-cancel, and block-and-confirm-then-run paths (see `prototype/build-order.md` M2)
- `[x]` **Conversational TUI** — full-screen persistent chat interface (bubbletea + lipgloss); accepts NL input, renders responses and command output; wraps CLI mode's already-working intent parser, classifier, and executor rather than rebuilding them. **Done Session 28** (`synapse tui`, `internal/tui`) — the propose/classify/confirm/execute loop is injected and driven, never reimplemented, so safety gating is identical across modes; adds token streaming, viewport scrollback, and the confirmation gate rendered as UI. See `prototype/build-order.md` M3b. **Caveat kept visible:** a full-screen TUI needs a real TTY, so this is the one deliverable no automated test can exercise end-to-end — `prototype/manual-tests/m3b-tui-mode.md` is the verification path.
...
- `[x]` **Intent parser client** — sends user utterance + session context to Ollama REST API; streams response tokens back to the TUI. **Done Session 28** — non-streaming `Generate` (M2) plus `GenerateStream` (M3b step 4, NDJSON token-by-token). Streaming is opt-in per mode: TUI streams, CLI/REPL deliberately do not (`docs/interface-modes.md`). A stream ending without a completion marker is rejected outright rather than returned short, so a truncated generation can never reach the classifier as a valid-but-different command.
- `[x]` **Bash execution engine** — dispatches shell commands as subprocesses via `os/exec`; captures stdout/stderr and surfaces the exit code. **Done 2026-07-12** (`internal/executor`), delivered via CLI mode first (`prototype/build-order.md` M2, previously scoped as a standalone M4); captures rather than streams for now — streaming to a live session is TUI-mode scope (M3b), reusing this same package
- `[x]` **Confirmation gate** — classifies pending commands as reversible/irreversible before dispatch; blocks irreversible commands pending explicit user approval. **Done 2026-07-12** (`internal/classifier`), delivered via CLI mode first (`prototype/build-order.md` M2, previously scoped as a standalone M5), reused by the TUI. **Rule set substantially extended since** (F1: `sed -i`/`awk -i inplace`/`truncate`/unsafe `tee`; F3/D22: recursive `chmod`/`chown`, fetch-decode-exec, bare `eval`) — see `prototype/build-order.md` F1/F3 and `../docs/decisions.md` D22 for the current full rule set
- `[~]` **Undo log** — records reversible operations; exposes an undo command that restores prior system state. **Mechanical half done** (F2, `internal/undo`, `synapse undo`) and **extended to confirmed irreversible commands too** (F5: content-backup, hardlink-based trash, metadata backup, and git-reset-SHA capture — see `../docs/safety-model.md` for the full mechanism-per-shape taxonomy). **Still open:** the telemetry half — logging `undo_invoked` events — gated on M7 (Session logger), see M8 in `prototype/build-order.md`
...
- `[x]` **Ollama connectivity check** — verifies Ollama is running at `localhost:11434` on startup; fails gracefully with a user-readable error if not. **Done and validated live 2026-07-12** (`internal/ollama.Client.Ping`, unit-tested against an unreachable server) — see `prototype/build-order.md` M2
...
- `[ ]` **Participant recruitment** — 20 participants (10 novice, 10 power user) across Windows, macOS, and Linux primary OS backgrounds; recruited through university channels after ethics approval; minimum 2 participants per OS background enforced as a recruitment quota floor (D18)
...
- `[x]` **Ch.3 citation integrity (SA3.1).** **Fixed 2026-07-08:** a full cross-chapter alignment audit found SA3.1's reference list carried five entries — `[3]` NaSh, `[5]` VoicePilot, `[16]` UFO², `[22]` GUIRoboTron-Speech, `[23]` R-VLM — never cited in the chapter's body text, violating D17's per-chapter citation-integrity rule. `[3]` and `[5]` are genuinely relevant to Ch.3's confirmation-gate/undo-log design (Ch.2 already names them as its motivation) and are now cited in Section 2.1b's safety-mechanism paragraph; `[16]` is now cited in the same section's GUI-out-of-scope paragraph, contrasting SynapseOS's deliberate CLI-only boundary against UFO²'s broader desktop-automation scope. `[22]` and `[23]` (speech-driven GUI grounding) have no legitimate connection to Ch.3's CLI methodology and were removed from SA3.1's reference list — they remain correctly cited in SA2 (Ch.2), where the underlying discussion lives.
- `[x]` **Ch.3 Introduction lacked a research-question restatement (SA3.1).** **Fixed 2026-07-08:** the rubric's Ch.3 "Introduction" row requires the research problem *and* question; SA3.1's opening paragraph restated only the general purpose. Added a paragraph restating RQ1–RQ3 from Ch.1 and mapping each to the Dataset/Procedure/System Testing sections that answer it.
- `[x]` **No Appendices section in SA3.1** for the SUS/NASA-TLX instrument copies the rubric grades (weight 3). **Fixed 2026-07-08:** added an Appendices section (Appendix A: SUS, Appendix B: NASA-TLX) with placeholder text — the finalized instrument copies are inserted once validated in the pilot study (Section 2.3), per the Study Instruments track in this file's Critical Path.
- `[x]` **Minor formatting deviations reviewed, accepted as-is (SA1/SA2/SA3.1), 2026-07-08.** A guidelines/rubric compliance audit found three pre-existing, low-stakes deviations from the ITRD writing-guidelines handout, all consistent across the chapters they appear in (not something Ch.2 introduced): (1) h3 (third-level) headings use Title Case instead of the handout's sentence case; (2) page numbers are positioned top-right on every page rather than the handout's bottom-center-on-major-section-pages / top-right-elsewhere rule — `tools/export_pdf.py`'s Chromium-based pipeline would need nontrivial per-page-type CSS to implement this correctly; (3) Chapter 1 uses discrete `h2` headings (Gap, Problem Statement, Objectives, Significance, Scope) rather than the handout's "continuous paragraph, no headings" instruction for the Introduction — deliberate since Session 1, since the rubric grades each of these as a separate row, but never previously logged as an intentional override the way D16 documents the citation-style override. Judgment call: fixing any of these three would cost real re-export/re-pagination effort for a cosmetic gain the rubric doesn't appear to grade; left as-is.
...
- `[x]` **PDF/TXT regeneration.** **Fixed 2026-07-04:** both PDFs re-exported from the corrected HTML via a headless Chromium render (Playwright + Liberation/DejaVu fonts, resolved entirely in user space — no root available in this environment). TXT mirrors deleted; HTML is now the sole source of truth (resolves the pending decision from `session.md`).
- `[x]` **D19/D20 (CLI mode, GUI-mode XFCE fallback) reflected in SA3.1.** Recorded 2026-07-10; archived pre-revision snapshot at `research-methods/archive/submitted-2026-07-10/`. **Fixed 2026-07-11 (`aa9fbee`):** Table 3.1 gets a fallback note on the SynapseOS machine row; Table 3.2 and the paragraph following it add CLI mode alongside TUI/GUI; Section 3.5 gets a new Internal Validity entry (Fallback reliance) describing the confound and its mitigation; new footnote 8 defines CLI mode. **Refined again 2026-07-12 (`bfe0e75`):** footnote 8 and the Table 3.2 paragraph updated to describe CLI mode as executing through the reversibility confirmation gate (not just proposing), matching the prototype's redefined M2 scope.
