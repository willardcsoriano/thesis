...
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.html` | Consolidated Ch.1–3 monograph — the actual Ch.2/Ch.3 submission | **Session 13: standalone Ch.2/Ch.3 drafts (`module 3/submissions/summative-assessment-2.html`, `-3.1.html`) deleted** — they were never individually submitted (only Ch.1 and this consolidated document were), and keeping both risked drift. This file is now hand-maintained directly; `tools/consolidate_proposal.py` (the generator that used to build it from the three chapter files) is retired. Content as of Session 12: 24-source RRL (Ch.2) + D19/D20-revised methodology (Ch.3, CLI mode + GUI fallback). Drafts preserved for reference only at `research-methods/archive/submitted-2026-07-10/` | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.pdf` | Consolidated Ch.1–3 monograph | Re-exported via `tools/export_pdf.py` directly (no longer via a consolidate script) | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3_paged.html` | Consolidated Ch.1–3 monograph (Paged.js variant) | **Session 11:** page-bottom footnotes via a vendored `paged.polyfill.js`. Does not resolve the Session 10 roman-numeral front-matter limitation below. **Session 13:** `tools/consolidate_proposal_paged.py` (its generator) retired along with the flat variant's — this file too is now hand-maintained; `tools/export_pdf_paged.py` (the exporter, distinct from the retired generator) is still used to render it | 2026-07-10 |
...
- **TOC page-number maintenance:** hardcoded, not self-updating — re-derive via `tools/export_pdf.py`'s `find_page()` before any future submission if content changes. Applies to SA1 (the one remaining standalone chapter) and the consolidated document. Full detail in `scope.md`'s Final Thesis Compilation section.
