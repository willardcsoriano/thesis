...
| `research-methods/module 3/submissions/summative-assessment-3.1.html` | Ch. 3 Methodology | Aligned with D9(revised)/D10/D12/D14/D15/D18, footnotes 1–7; TOC/LOT/LOF + anchor ids. Renumbered Ch.2→Ch.3 (Session 5); §1.4 names the **convergent mixed-methods** design (PR #5). **Session 9:** fixed broken citation integrity, added RQ1–RQ3 restatement, Appendices section, D18 sentence. **Session 12:** adds D19 (CLI mode) and D20 (participant-accessible GUI-mode XFCE fallback, logged and excluded from primary analysis) to Table 3.1, Table 3.2, the paragraph following it, and a new Section 3.5 "Fallback reliance" Internal Validity entry; footnote 8 added. **This revises already-submitted/graded content** — the pre-revision version is archived at `research-methods/archive/submitted-2026-07-10/` and tagged `submitted-2026-07-10` in git | 2026-07-10 |
| `research-methods/module 3/submissions/summative-assessment-3.1.pdf` | Ch. 3 Methodology | Re-exported via `tools/export_pdf.py` — TOC/LOT/LOF page numbers re-derived and verified against the new content (38 pages, up from 35, Session 12) | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.html` | Consolidated Ch.1–3 monograph | Built by `tools/consolidate_proposal.py` from the three chapter files above plus templated front matter. **Session 10:** fixed a stale/corrupted build. **Session 11:** relocated to `research-methods/consolidated/`, generator script tracked in git for the first time. **Session 12:** rebuilt against the D19/D20-revised Ch.3; caught and fixed three hardcoded chapter-relative TOC offsets (Table 3.2, Figure 3.1, Section 3) that had drifted out of sync with the new content — re-verified against the rebuilt PDF via `find_page()`, not assumed | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.pdf` | Consolidated Ch.1–3 monograph | Re-exported via `tools/consolidate_proposal.py`'s two-pass build, all TOC/LOT/LOF entries spot-verified against `pdftotext` output (Session 12) | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3_paged.html` | Consolidated Ch.1–3 monograph (Paged.js variant) | **Session 11:** built by `tools/consolidate_proposal_paged.py`, page-bottom footnotes via a vendored `paged.polyfill.js`. Does not resolve the Session 10 roman-numeral front-matter limitation below. **Session 12:** rebuilt against D19/D20; its hardcoded TOC offsets needed **different values from the flat variant** (page-bottom footnotes consume more vertical space per page) — computed and verified independently, not copied across | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3_paged.pdf` | Consolidated Ch.1–3 monograph (Paged.js variant) | Re-exported via `tools/export_pdf_paged.py`, waits on `window.pagedFinished` before printing (Session 12) | 2026-07-10 |
...
- **Consolidated proposal (`tools/consolidate_proposal.py` and `_paged.py`) has a hybrid page-numbering scheme:** the chapter/front-matter *starts* (APR/ACK/TOC/LOT/LOF/ABS/CH1/CH2/CH3/REF/APP) are auto-detected via the two-pass build and self-correct on any content change; the *sub-entries within* Chapter 2 and Chapter 3 (e.g. `2.8 Synthesis`, `Table 2.1`, `Section 3: System Testing`) are still hardcoded chapter-relative offsets (`p_ch2 + N`, `p_ch3 + N`) and will silently drift if ch2/ch3 content changes again. **This predicted drift actually happened in Session 12** (Table 3.2/Figure 3.1/Section 3 all desynced when Ch.3 grew for D19/D20) and was fixed by re-deriving fresh offsets — but the underlying hardcoding is still there for next time. Re-verify with `pdftotext -layout` (or `find_page()` against distinctive body-text snippets — bare heading titles collide with the TOC's own text) against the rebuilt PDF before any future submission of the consolidated file, **and treat the flat and paged variants as needing independent verification** — Session 12 found their correct offsets differ by up to 10 pages for the same heading, since page-bottom footnotes consume different vertical space than the flat layout.
- **Front matter still uses continuous Arabic page numbers, not the roman-numeral-then-restart scheme the writing guideline's sample TOC shows** (flagged Session 10, confirmed still open Session 11). The Paged.js pipeline added in Session 11 (`consolidate_proposal_paged.py`) was built for page-bottom footnotes, not this — it does not implement `counter-reset`/roman-numeral CSS either. If this matters for grading, it's a real fix to make (Paged.js can do it; the CSS just hasn't been written), not a Chromium limitation anymore now that a Paged.js path exists.
- **`consolidate_proposal.py` and `consolidate_proposal_paged.py` are ~70% duplicated** (431 vs. 486 lines, most of the front-matter templating and page-detection logic copy-pasted rather than shared) — minor maintenance smell from Session 11's fast turnaround, worth factoring into one script with a `--paged` flag if either gets touched again.
...
| File | Purpose |
|------|---------|
| `decisions.md` | All architecture and study design decisions (D1–D18) |
...
| `tools/` | Self-contained PDF export tooling — `export_pdf.py` (venv + Playwright), `consolidate_proposal.py` (builds `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.html/pdf` from the three chapter files + templated front matter), `consolidate_proposal_paged.py` + `export_pdf_paged.py` (Paged.js variant with page-bottom footnotes), `TOOLING.md` (what's Python vs. vendored native vs. irreducible), `requirements.txt`; paired with the repo-root `Makefile` |
