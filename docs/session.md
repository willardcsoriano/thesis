...
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.html` | Consolidated Ch.1–3 monograph — **the sole living document, as of Session 16** | **Session 14:** HTML source retired (frozen as historical snapshot) when the paged variant became the sole living document. **Session 16:** revived as the living document — the Paged.js same-page-footnote pipeline was retired (see `research-methods/archive/paged-footnotes-attempt/README.md` for the full reasoning: ~35% page-density loss from the polyfill's own layout imprecision, not just genuine footnote-space overhead). Restored from git history (the exact pre-retirement state), then given the same fixes already proven on the paged variant in Session 15 — 7 duplicate anchor `id`s namespaced, 2 genuinely orphaned headings fixed (here, native Chromium `page-break-after: avoid` works correctly, no workaround needed), all 37 TOC/LOT/LOF entries re-derived against the font-fixed 43-page PDF | 2026-07-12 |
