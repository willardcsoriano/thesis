...
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.html` | Consolidated Ch.1–3 monograph | Built by `tools/consolidate_proposal.py` from the three chapter files above plus templated front matter (Approval Page, Acknowledgement, Abstract). **Session 10:** fixed a stale/corrupted build (front-matter TOC drift, Appendices/References ordering, orphaned `<div>`s). **Session 11:** relocated from `research-methods/` to `research-methods/consolidated/`; the generator script itself was tracked in git for the first time (previously only its output was committed); a stale template string reverting the Session-shipped department/degree fix (Computer Science → Information Technology) was caught and fixed before commit. Merged to `main` via PR #9 | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.pdf` | Consolidated Ch.1–3 monograph | Re-exported via `tools/consolidate_proposal.py`'s two-pass build — 41 pages, all TOC/LOT/LOF entries spot-verified against `pdftotext` output | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3_paged.html` | Consolidated Ch.1–3 monograph (Paged.js variant) | **Session 11:** built by the new `tools/consolidate_proposal_paged.py`, rendered through a vendored `paged.polyfill.js` instead of Chromium's built-in print-to-PDF. Adds real page-bottom footnotes (`float: footnote`) that Chromium's print engine can't do natively. **Does not** resolve the Session 10 roman-numeral front-matter limitation below — that's still open | 2026-07-10 |
