...
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3.pdf` | Consolidated Ch.1–3 monograph (flat variant) | **Session 14: HTML source retired**, same reasoning — this PDF already carries the Session 12 D19/D20 content (rebuilt before the generator was deleted in Session 13), so it's frozen as a historical snapshot, not stale. The flat variant is no longer maintained at all going forward; the paged variant below is now the **sole** living document | 2026-07-10 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3_paged.html` | Consolidated Ch.1–3 monograph — **the sole living document, as of Session 14** | **Session 11:** page-bottom footnotes via a vendored `paged.polyfill.js` — genuine `float: footnote` rendering, verified working (footnote markers and their definitions land on the same page). **Session 13:** its generator (`consolidate_proposal_paged.py`) retired; hand-maintained directly since, `tools/export_pdf_paged.py` (the exporter, not a generator) still renders it. **Session 14:** now the only consolidated document maintained at all — Ch.1's standalone file and the flat consolidated variant were both retired to frozen PDFs this session, closing out the footnote-rendering problem by elimination rather than by patching every pipeline. **Session 15:** fixed the actual font-rendering bug this exposed (see below), namespaced 21 colliding anchor `id`s, added `break-inside: avoid` wrappers around 3 orphaned headings, and re-derived all 36 TOC/LOT/LOF page numbers against the resulting 62-page PDF. Still does not resolve the Session 10 roman-numeral front-matter limitation below | 2026-07-12 |
| `research-methods/consolidated/SynapseOS_Proposal_Chapters_1_to_3_paged.pdf` | Consolidated Ch.1–3 monograph (Paged.js variant) | **Session 15:** re-exported with Liberation Serif now actually resolving (was silently falling back to DejaVu Sans, see below) — 62 pages, all TOC/LOT/LOF entries verified against `pdftotext` output. Waits on `window.pagedFinished` before printing | 2026-07-12 |
...
- **TOC page-number maintenance:** hardcoded, not self-updating — re-derive via `tools/export_pdf_paged.py`'s `find_page()` before any future edit changes content. Applies only to the paged consolidated document now (Session 14) — there is no other live chapter source left. Full detail in `scope.md`'s Final Thesis Compilation section.
- **Consolidated proposal page numbering is now fully manual, not partially self-correcting.** Before Session 13, `tools/consolidate_proposal.py`'s two-pass build auto-detected the chapter/front-matter *starts* (APR/ACK/TOC/LOT/LOF/ABS/CH1/CH2/CH3/REF/APP) and only left sub-entries within Chapter 2/3 as hardcoded offsets. That script is retired (Session 13) — the consolidated HTML is hand-maintained directly now, so **every** TOC/LOT/LOF entry, including the ones that used to self-correct, needs manual re-derivation via `find_page()` after any future edit. Search against distinctive body-text snippets, not bare heading titles — bare titles collide with the TOC's own text and return false early-page matches. Treat the flat and paged variants as needing fully independent verification — Session 12 found their correct offsets differ by up to 10 pages for the same heading, since page-bottom footnotes consume different vertical space than the flat layout.
- **Front matter still uses continuous Arabic page numbers, not the roman-numeral-then-restart scheme the writing guideline's sample TOC shows** (flagged Session 10, confirmed still open Session 11). The Paged.js-rendered variant was built for page-bottom footnotes, not this — it does not implement `counter-reset`/roman-numeral CSS either. If this matters for grading, it's a real fix to make directly in the hand-maintained paged HTML (Paged.js can do it; the CSS just hasn't been written), not a Chromium limitation anymore now that a Paged.js rendering path exists.
...
| `tools/` | Self-contained PDF export tooling — `export_pdf.py` (venv + Playwright, plain Chromium print-to-PDF) and `export_pdf_paged.py` (Paged.js variant, page-bottom footnotes) both take any HTML file directly; `TOOLING.md` (what's Python vs. vendored native vs. irreducible), `requirements.txt`; paired with the repo-root `Makefile`. The `consolidate_proposal*.py` generator scripts that used to build the consolidated proposal from separate chapter files were retired in Session 13 — it's hand-maintained directly now |
| `research-methods/consolidated/` | Output directory for the consolidated Ch.1–3 monograph — flat build + Paged.js build side by side, plus the vendored `paged.polyfill.js` |
